plot_phases <- function(tagdat, mdists, cee.times, which_vars, thr, sim,
                        hourlim, withlegend, phases=TRUE){
  ...
  par(mar=c(1,8,0,2), oma=c(8,10,3,0), las=1, lwd=3, cex=1,
...
#      font=2, col="white")
 #     col.axis="black", col.lab="black",
  ...
  if (length(which_vars) < 3) {par(cex.lab=5, cex.axis=5)}
  colrs <- c("white", brewer.pal(8,"Set2"))
  pcols <- cbPalette[2:6]
  acols <- c('white', brewer.pal(6, "RdBu")[1:2])
  ...
  axis(side=1,labels=FALSE,tick=TRUE, col='white')
  axis(side=2,labels=TRUE,tick=TRUE, col='white')
  if (length(which_vars) < 3){
    mtext("Depth (m)", side=2, line=11, cex=6, las=3)
  }else{
  ...
  plot(hr, tagdat$Ax, type="l", col=acols[1],
       ylim=c(-1.2, 1.2), xlim=xl,
      ylab="" ,
       xlab="", xaxt="n", yaxt="n")
  lines(hr, tagdat$Ay, type="l", col=acols[2])
  lines(hr, tagdat$Az, type="l", col=acols[3])
  axis(col='white', side=1,labels=FALSE,tick=TRUE)
  axis(col='white', side=2,labels=TRUE,tick=TRUE)
  ...
  if (!plotmd){
    axis(col='white', side=1,labels=TRUE,tick=TRUE, padj=1)
    if ("verticalvelocity" %in% which_vars){
    par(las=0)
    axis(col='white', side=2,labels=TRUE,tick=TRUE)
    }
    if (length(which_vars) < 3){
      mtext("Time (h)", side=1, line=6, cex=5)
    }else{
    mtext("Time (h)", side=1, line=4, cex=4)
    }
  }else{
    axis(col='white', side=1,labels=TRUE,tick=TRUE)
    axis(col='white', side=2,labels=FALSE,tick=TRUE)
  }
  ...
  if (plotmd){
    ...
    axis(col='white', side=2,labels=TRUE,tick=TRUE)
    if (length(which_vars) < 3){
      mtext("Time (h)", side=1, line=6, cex=6)
    }else{
    mtext("Time (h)", side=1, line=4, cex=4)
    }
  }
